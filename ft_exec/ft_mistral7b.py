...
training_args = TrainingArguments(
    output_dir='./ft_models/mistral7b',
    num_train_epochs=3,
...
trainer.train()
new_model = "ft_mistral7b_new"
trainer.model.save_pretrained(new_model)
tokenizer.save_pretrained(new_model)

if parse_args.no_ctx:
    test_lines = open('./data/mistral_data/test_mistral_no_ctx_lab.txt').readlines()
else:
    test_lines = open('./data/mistral_data/test_mistral_lab.txt').readlines()
...
for prompt in tqdm(test_lines):
    ...
    outputs = model.generate(input_ids=inputs["input_ids"].to("cuda"), max_new_tokens=512, temperature=0.5, num_return_sequences=1)
